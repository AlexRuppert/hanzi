...
function start(withComponent = true) {
  var i = 0;
  var lines = [];
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    var components = decomposition.substring(openbracket + 1, closebracket);
...
function getComponents(character) {
  if (ifComponentExists(character)) {
    /*if (characters[character].typeOfDecomposition) {
      return character;
    } else {*/
    return characters[character].split(',');
    // }
  } else {
    return character;
  }
